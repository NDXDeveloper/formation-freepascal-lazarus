...
begin
  LibSystemd := LoadLibrary('libsystemd.so');
  if LibSystemd <> 0 then
  begin
...
begin
  ...
  FLock.Enter;
  try
    Output := Format('[%s]', [LevelToString(Level)]);
...
begin
  ...
  SanitizedMsg := TRegEx.Replace(SanitizedMsg, '\d{4}[\s-]?\d{4}[\s-]?\d{4}[\s-]?\d{4}', 'XXXX-XXXX-XXXX-XXXX');

  // Masquer les emails partiellement
  SanitizedMsg := TRegEx.Replace(SanitizedMsg, '([^@]{2})[^@]*@', '$1***@');
...
uses
  SysUtils, Classes, IdHTTP, fpjson, jsonparser;
...
begin
  ...
  try
    ...
    // Envoyer à Zabbix
    try
      FHTTPClient.Post(FZabbixServer + '/api/v1/sender', Payload);
