  ...
  begin
    ...
    begin
      ...
      if Assigned(FOnTimeout) then
        Synchronize(procedure begin FOnTimeout(Self); end);
    end;
...
```pascal
type
  TPerformanceCounters = class
  private
    FCounterProcessTime: Int64;
    FCounterMemoryUsage: Int64;
    FCounterRequestsPerSec: Integer;
    FCounterActiveConnections: Integer;
    FLastUpdate: TDateTime;

    procedure RegisterCounters;
...
procedure TPerformanceCounters.RegisterCounters;
const
  CategoryName = 'Mon Service FreePascal';
  CategoryHelp = 'Compteurs de performance pour Mon Service';
var
  CounterData: TCounterCreationDataCollection;
begin
  // Créer la catégorie de compteurs si elle n'existe pas
  if not PerformanceCounterCategory.Exists(CategoryName) then
  begin
    CounterData := TCounterCreationDataCollection.Create;
    try
      // Compteur : Requêtes par seconde
      CounterData.Add(TCounterCreationData.Create(
        'Requêtes/sec',
        'Nombre de requêtes traitées par seconde',
        PerformanceCounterType.RateOfCountsPerSecond32
      ));

      // Compteur : Connexions actives
      CounterData.Add(TCounterCreationData.Create(
        'Connexions actives',
      ...
      // Compteur : Utilisation mémoire
      CounterData.Add(TCounterCreationData.Create(
        'Mémoire (MB)',
        'Utilisation mémoire en mégaoctets',
        PerformanceCounterType.NumberOfItems32
      ));
